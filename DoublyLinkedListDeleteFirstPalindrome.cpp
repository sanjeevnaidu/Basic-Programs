...
void deletePalindrome(node **start,node **end){
	node *temp;
	int n, rev = 0;
	for(temp = *start; temp != NULL; temp=temp->next){
		n=temp->no;
		if(isPalindrome(n) == 1){
			if(temp->prev == NULL)
		 	...
		 	else
			  {
			   ...
			   printf("\n\n\t Element [%d] is deleted from Linklist",temp->no);
			   free(temp);
			   getch();
			  }
			return;
	...
	printf("\n\n\t No palindrome found in the Doubly Linked List");
	getch();
	return;
}
	...
	while(1)
	 {
	  ...
	  printf("\n\t 7. To delete a particuar node of doubly linked list");
	  printf("\n\t 8. To traverse and delete first palindrome node");
	  printf("\n\t 9. Exit");
	  printf("\n\n\t  Enter your choice ==> ");
	  ch=getchar();
	  switch(ch)
